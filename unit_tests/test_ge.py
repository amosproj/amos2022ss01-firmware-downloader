...
class GEUnitTest(unittest.TestCase):
    def test_case_without_authentication(self):
        ...
        if os.path.isfile(gt_file_path) is False and check_duplicates(data, DB_NAME) is True:
            download_file(gt_url, gt_file_path, files[0], files[1], file_name, '', '', '', DB_NAME, True)
        else:
            download_file(gt_url, gt_file_path, files[0], files[1], file_name, '', '', '', DB_NAME, False)
    ...
    def test_case_with_authentication(self):
        ...
        if os.path.isfile(gt_ex_file_path) is False and check_duplicates(data, DB_NAME) is True:
            download_file(gt_url, gt_file_path, files[0], files[1], file_name, 'javascript:;', gt_url, "Passport_DownloadFile('SDSeries',7,70);return false", DB_NAME, True)
        else:
            download_file(gt_url, gt_file_path, files[0], files[1], file_name, 'javascript:;', gt_url, "Passport_DownloadFile('SDSeries',7,70);return false", DB_NAME, False)
